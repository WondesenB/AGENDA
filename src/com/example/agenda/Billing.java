...
public class Billing extends Activity {
	...
	private void addItem() {
		// TODO Auto-generated method stub
		li = getResources().getStringArray(R.array.bill_categroy);
	...
	protected void submitData() {
		...
		try {
			db.open();
			num = db.addbill_info(ti, cat, sdate, stime, stime_class, edate,
		...
		if (num > 0) {
			Toast.makeText(this,
					getResources().getString(R.string.bill_event_added), 2000)
					.show();
			title_edit.setText("");
			...
			Toast.makeText(this,
					getResources().getString(R.string.error_dublicat), 4000)
					.show();
		else
			Toast.makeText(this,
					getResources().getString(R.string.error_insert), 2000)
					.show();
	...
	private void updateData() {
		...
		try {
			db.open();
			db.updateRow_bill(ti, cat, sdate, stime, stime_class, edate, etime,
					etime_class, fr, note, caltype, tit_, categ_, sDate_,
					sTime_, sTclass_, eDate_, eTime_, eTclass_, freq_, nott_);
			db.close();
		} catch (SQLException e) {
			num = -5;
		} finally {
			db.close();
			Toast.makeText(this, getResources().getString(R.string.updated),
					2000).show();
		}
